// MongoDB Seed Script for VentasSaaS
// Database: VentasSaaS (Massive Seeding)

const dbName = "VentasSaaS";
const conn = new Mongo();
const db = conn.getDB(dbName);

print("Starting MASSIVE seeding for VentasSaaS...");

// Clean up existing transaction and master data (preserve Users and Empresas)
...
db.StockMovements.deleteMany({});

print("Cleared old transaction and catalog data.");

const companies = [
  {
    id: ObjectId("6a297aca2e7ef220b17ac96c"),
    name: "TU TIENDA CHEVERE",
    ownerId: ObjectId("6a297aca2e7ef220b17ac96d"),
    ownerName: "marcelo",
    type: "bakery"
  },
  {
    id: ObjectId("6a297e7cd0136764058f1f8e"),
    name: "LA MEJOR",
    ownerId: ObjectId("6a297e7cd0136764058f1f8f"),
    ownerName: "alicia",
    type: "boutique"
  },
  {
    id: ObjectId("6a2987a46f2cef91877179a7"),
    name: "Heladeria Antigravity",
    ownerId: ObjectId("6a2987a46f2cef91877179a8"),
    ownerName: "Juan Perez",
    type: "icecream"
  }
];

// Helper to generate dates distributed over the last 90 days
function getRandomDateInLast90Days(dayOffset) {
  const d = new Date();
  d.setDate(d.getDate() - dayOffset);
  d.setHours(Math.floor(Math.random() * 12) + 8); // between 8am and 8pm
...
companies.forEach(company => {
  print("Seeding company: " + company.name);
  ...
  // 6. Seed Sales (180 sales per company over last 90 days)
  for (let i = 1; i <= 180; i++) {
    const date = getRandomDateInLast90Days(90 - i * 0.48);
  ...
  print("Completed massive seeding for: " + company.name);
});

print("All companies successfully populated with MASSIVE amounts of data!");
