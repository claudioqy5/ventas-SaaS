...
<!-- Barra lateral -->
    ...
    <HamsterLoader v-if="loading" label="Cargando formas de pago..." />
      ...
      <div v-if="!loading" class="card font-card">
        <div v-if="!loading && paymentMethods.length === 0" class="empty-state">
          ...
          <form @submit.prevent="savePaymentMethod">
            ...
            <div v-if="isEdit" class="field switch-field">
              <label>
                ...
                Esta forma de pago estÃ¡ activa
              </label>
            </div>
...
const confirmDelete = async (id) => {
  if (!confirm('Â¿EstÃ¡s seguro de eliminar esta forma de pago permanentemente?')) return
  try {
    const res = await fetch(`${API_URL}/api/paymentmethods/${id}`, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${authStore.token}` }
    })
    if (!res.ok) throw new Error('No se pudo eliminar. Si estÃ¡ en uso, contacte a soporte.')
