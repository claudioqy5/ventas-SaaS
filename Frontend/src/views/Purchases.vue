...
<!-- Barra de navegacion lateral -->
    ...
    <HamsterLoader v-if="loading" label="Cargando abastecimiento y compras..." />
      ...
      <div v-if="showRegisterForm" class="card font-card form-container">
        <h2 class="section-title">âž• Nueva Compra / Ingreso de AlmacÃ©n</h2>
        ...
        <form @submit.prevent="submitPurchase" class="grid">
          ...
          <div class="form-footer">
            ...
            <button type="submit" class="btn btn-success" :disabled="form.detalles.length === 0 || loading">
              {{ loading ? 'Procesando...' : 'ðŸ“¥ Confirmar e Ingresar a AlmacÃ©n' }}
            </button>
          </div>
        </form>
      </div>
      ...
      <div v-if="!loading" class="card font-card">
        ...
        <div v-if="!loading && filteredPurchases.length === 0" class="empty-state">
...
const submitPurchase = async () => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(err.message || 'Error al procesar el ingreso de almacÃ©n.')
    }

    alert('Â¡Compra registrada e inventario actualizado con Ã©xito!')
    form.proveedorId = ''
    form.detalles = []
