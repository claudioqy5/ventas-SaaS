<template>
  <div class="dashboard-layout">
    ...
    <main class="main-content">
      ...
      <div v-else style="display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 12px; margin-bottom: 24px;">
        <!-- Venta Total Cobrada -->
        <div class="kpi-total-card bruto" style="background: linear-gradient(135deg, #eef2ff, #e0e7ff); border: 1px solid #c7d2fe; padding: 14px 16px; border-radius: var(--radius-md); text-align: left; box-shadow: var(--shadow-sm);">
          ...
          <div style="font-size: 1.35rem; font-weight: 700; color: #1e1b4b; margin-top: 6px;">S/. {{ (stats.totalIngresos || 0).toFixed(2) }}</div>
          <div style="font-size: 0.7rem; color: #6366f1; margin-top: 2px;">{{ stats.totalVentas || 0 }} ventas registradas</div>
        </div>

        <!-- Ganancia Real (Post-Impuestos y Costos) -->
        <div class="kpi-total-card neto" style="background: linear-gradient(135deg, #f0fdf4, #dcfce7); border: 1px solid #86efac; padding: 14px 16px; border-radius: var(--radius-md); text-align: left; box-shadow: var(--shadow-sm);">
          <div style="font-size: 0.7rem; font-weight: 700; color: #15803d; text-transform: uppercase; letter-spacing: 0.5px; display: flex; align-items: center; gap: 4px;">
          ...
          <div style="font-size: 1.35rem; font-weight: 700; color: #14532d; margin-top: 6px;">S/. {{ (stats.gananciaReal || stats.gananciaBruta || 0).toFixed(2) }}</div>
          <div style="font-size: 0.7rem; color: #16a34a; margin-top: 2px;">Desc. IGV y Costos</div>
        </div>

        <!-- Impuesto IGV acumulado para SUNAT -->
        <div class="kpi-total-card igv" style="background: linear-gradient(135deg, #fef3c7, #fde68a); border: 1px solid #fcd34d; padding: 14px 16px; border-radius: var(--radius-md); text-align: left; box-shadow: var(--shadow-sm);">
          <div style="font-size: 0.7rem; font-weight: 700; color: #b45309; text-transform: uppercase; letter-spacing: 0.5px; display: flex; align-items: center; gap: 4px;">
          ...
          <div style="font-size: 1.35rem; font-weight: 700; color: #78350f; margin-top: 6px;">S/. {{ (stats.totalIgv || 0).toFixed(2) }}</div>
          <div style="font-size: 0.7rem; color: #d97706; margin-top: 2px;">Generado (SUNAT)</div>
        </div>

        <!-- Card: Notas de Venta -->
        <div class="kpi-total-card nv" style="background: white; border: 1px solid #e2e8f0; border-left: 4px solid #64748b; padding: 14px 16px; border-radius: var(--radius-md); text-align: left; box-shadow: var(--shadow-sm);">
          <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 0.7rem; font-weight: 700; color: #475569; text-transform: uppercase;">Notas de Venta</span>
            <span style="background: #f1f5f9; color: #475569; font-size: 0.65rem; font-weight: 700; padding: 2px 6px; border-radius: 10px;">{{ stats.countNotasVenta || 0 }} und</span>
          </div>
          <div style="font-size: 1.35rem; font-weight: 700; color: #0f172a; margin-top: 6px;">S/. {{ (stats.totalNotasVenta || 0).toFixed(2) }}</div>
          <div style="font-size: 0.7rem; color: #94a3b8; margin-top: 2px;">Comp. interno (Sin IGV)</div>
        </div>

        <!-- Card: Boletas de Venta -->
        <div class="kpi-total-card boletas" style="background: white; border: 1px solid #e2e8f0; border-left: 4px solid #2563eb; padding: 14px 16px; border-radius: var(--radius-md); text-align: left; box-shadow: var(--shadow-sm);">
          <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 0.7rem; font-weight: 700; color: #1d4ed8; text-transform: uppercase;">Boletas</span>
            <span style="background: #dbeafe; color: #1e40af; font-size: 0.65rem; font-weight: 700; padding: 2px 6px; border-radius: 10px;">{{ stats.countBoletas || 0 }} und</span>
          </div>
          <div style="font-size: 1.35rem; font-weight: 700; color: #1e3a8a; margin-top: 6px;">S/. {{ (stats.totalBoletas || 0).toFixed(2) }}</div>
          <div style="font-size: 0.7rem; color: #3b82f6; margin-top: 2px;">Electrónica (18% IGV)</div>
        </div>

        <!-- Card: Facturas -->
        <div class="kpi-total-card facturas" style="background: white; border: 1px solid #e2e8f0; border-left: 4px solid #7c3aed; padding: 14px 16px; border-radius: var(--radius-md); text-align: left; box-shadow: var(--shadow-sm);">
          <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 0.7rem; font-weight: 700; color: #6d28d9; text-transform: uppercase;">Facturas</span>
            <span style="background: #f3e8ff; color: #6b21a8; font-size: 0.65rem; font-weight: 700; padding: 2px 6px; border-radius: 10px;">{{ stats.countFacturas || 0 }} und</span>
          </div>
          <div style="font-size: 1.35rem; font-weight: 700; color: #4c1d95; margin-top: 6px;">S/. {{ (stats.totalFacturas || 0).toFixed(2) }}</div>
          <div style="font-size: 0.7rem; color: #8b5cf6; margin-top: 2px;">Electrónica (18% IGV)</div>
        </div>
      </div>
