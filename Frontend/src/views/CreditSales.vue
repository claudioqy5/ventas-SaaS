    ...
    <HamsterLoader v-if="loading" label="Cargando cuentas por cobrar..." />
      ...
      <template v-if="activeTab === 'pendientes'">
        <div v-if="!loading" class="card font-card">
          <div v-if="!loading && pendingSales.length === 0" class="empty-state">
            No tienes cuentas por cobrar pendientes en este momento.
          </div>
          <table v-if="!loading && pendingSales.length > 0" class="data-table">
            <thead>
              <tr>
      ...
      <template v-if="activeTab === 'historico'">
        <div class="card font-card">
          <div v-if="!loading && historySales.length === 0" class="empty-state">
            AÃºn no hay registros de fiados que hayan sido pagados.
          </div>
          <table v-if="!loading && historySales.length > 0" class="data-table">
            <thead>
              <tr>
...
const submitPayment = async () => {
  ...
  try {
    ...
    alert('Pago registrado correctamente. Â¡El dinero ya cuenta en las estadÃ­sticas!')
